...
async function createFragments (opts) {
    const {
      ...
      for (let i in videos) {
        let vid = videos[i];
        let curTrans = transitions[i] || transition;
      ...
      await Promise.all([
        pmap(params, createBaseFile, { concurrency }),
        pmap(params, createBeginFile, { concurrency }),
        pmap(params, createEndFile, { concurrency })
      ])
      .then(log)
      .catch(console.error)

      // use ffmpeg-concat to transition between Begin and End files


      // use ffmpeg to standard concat everything in order
...
async function createBeginFile (opts) {
  ...
  try {
    if (isLast) {
      ...
      durationTimestamp = convertMStoTimestamp(cutMiliseconds)
      output = path.join(outputDir, `${index}.begin${ext}`)
  ...
  try {
    if (isFirst) {
      ...
      // Create the fragment the needs an outro transition
        ...
        output = path.join(outputDir, `${index}.end${ext}`)
        transitionFragments.push(await createCopy(video, beginTimestamp, null, output))
